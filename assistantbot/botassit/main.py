...
class ConsoleInterface(UserInterface):
    def display_contacts(self, contacts):
        ...
        for contact in contacts:
            print(
                f"{contact.name} | {contact.address} | {contact.phone} | {contact.email} | {contact.birthday}"
            )
...
class BotAssist:
    ...
    def add_contact(self, name, address, phone, email, birthday):
        ...
        for contact in self.contacts:
            if (
                ...
                and contact.birthday.lower() == birthday.lower()
            ):
                print(
                    f"Contact with name '{name}' and birthday '{birthday}' already exists. Can not duplicate contact."
                )
                return
    ...
    def add_note(self, note_name, note_text):
        if note_name in self.notes:  # Перевірка, чи назва нотатки вже існує
            choice = input(
                f"Note '{note_name}' already exists. Do you want to edit it? enter yes or no: "
            ).lower()
            if choice == "yes":
                self.edit_note(
                    note_name, note_text
                )  # Виклик методу для редагування нотатки
            else:
                print("Note creation aborted.")
        else:
            self.notes[note_name] = Note(
                note_text
            )  # Створення нової нотатки в словнику notes
    ...
    def delete_note(self, note_name):
        if not self.notes:  # Перевірка, чи словник notes пустий
            ...
            print(f"Note '{note_name}': {self.notes[note_name].text}")
            choice = input(
                f"Are you sure you want to delete note '{note_name}'? (1 - Yes, 2 - No): "
            )
            if choice == "1":
    ...
    def load_data(self, filename):
        try:
            with open(filename, "rb") as file:
                ...
                self.contacts = data[0] if data and isinstance(data, tuple) else []
                self.notes = data[1] if data and isinstance(data, tuple) else {}
                self.tags = data[2] if data and isinstance(data, tuple) else {}
...
def main():
    ...
    while True:
        command = input(
            "\nEnter your command for start(for menu-press 'menu'): "
        ).lower()
        ...
        if command == "1":
            ...
            search_query = console_interface.get_user_input(
            ...
            contact_name = console_interface.get_user_input(
                "Enter the contact name you want to delete:"
            )
            assistant.delete_contact(contact_name)

        elif command == "4":
            old_contact_name = console_interface.get_user_input(
                "Enter the contact old name you want to edit: "
            )
            ...
            new_birthday = console_interface.get_user_input(
                "Enter the new birthday in YYYY-MM-DD: "
            )
            print(
            ...
            day_to_birthday = int(
                console_interface.get_user_input(
                    "Enter the number of days until the birthday: "
                )
            )
            ...
            edit_or_delete = console_interface.get_user_input(
                "Enter 'edit' to edit a note or 'delete' to delete a note: "
            ).lower()

            if edit_or_delete == "edit":
                note_name = console_interface.get_user_input(
                    "Enter note name to edit: "
                )
                new_text = console_interface.get_user_input(
                    "Enter new text for the note: "
                )
                assistant.edit_note(note_name, new_text)
            elif edit_or_delete == "delete":
                note_name = console_interface.get_user_input(
                    "Enter note name to delete: "
                )
                assistant.delete_note(note_name)
            else:
                console_interface.display_message(
                    "Invalid command. Please enter 'edit' or 'delete'."
                )

        elif command == "9":
            title = console_interface.get_user_input("Enter note name:")
            new_tags = console_interface.get_user_input(
                "Enter tags (comma separated):"
            ).split(",")
            assistant.add_tags_to_note(title, new_tags)

        elif command == "10":
            tags = console_interface.get_user_input(
                "Enter tags for search (comma separated):"
            ).split(",")
            ...
            folder_path = console_interface.get_user_input(
                "Enter the folder path to sort :"
            )
            assistant.sort_files(folder_path)
            console_interface.display_message("Folder is sorted successfully!")

        elif command == "save":
            filename = console_interface.get_user_input(
            ...
            filename = console_interface.get_user_input(
                "Enter the filename to load data: "
            )
